<template>
  <nav class="flex rounded-md" aria-label="Pagination">
    ...
    <!-- Previous -->
    <button @click="gotoPage(page - 1)" v-if="page != 1" href="#" class="flex items-center justify-center px-3 h-8 leading-tight border border-e-0 text-gray-500 bg-white border-gray-300 hover:bg-gray-100 hover:text-gray-700 rounded-s-lg">
    ...
    <!-- First -->
    <button @click="gotoPage(1)" v-if="page > 2" aria-current="page" class="flex items-center justify-center px-3 h-8 leading-tight border border-e-0 text-gray-500 bg-white border-gray-300 hover:bg-gray-100 hover:text-gray-700">1</button>
  ...
  methods: {
    getNumbers() {
      ...
      for (
        ...
        i <= Math.min(this.page + 1, this.pageCount);
        i++
      ) {
        res.push(i);
      }
